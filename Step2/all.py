# 在243行选择用哪个类做实例化，CSNN是原网络，CSNNP是加了一个卷积层和一个全连接层，CSNNPwithAtt是在CSNNP基础上加了注意力机制
...
def main():
    ...
    net = CSNN(T=args.T, channels=args.channels, use_cupy=args.cupy) # 在这里选择具体用哪个类做实例化
    ...
    # 定义一个转换函数，将灰度图像转换为彩色图像
    class GrayscaleToRGB(object):
        def __call__(self, img):
            img = np.array(img)  # 将 PIL 图像转换为 NumPy 数组
            if img.ndim == 2:  # 如果是灰度图像
                img = np.stack((img,)*3, axis=-1)  # 将灰度图像复制到三个通道
            img = img.astype(np.uint8)  # 确保数据类型为 uint8
